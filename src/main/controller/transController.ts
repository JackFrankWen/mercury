      ...
      if (existingParent) {
        ...
        return newList.map((item) => {
          if (item.id === parent_id) {
            return {
              ...item,
              value: roundToTwoDecimalPlaces(Number(item.value) + Number(element.total)),
              avg: roundToTwoDecimalPlaces(Number(item.avg) + Number(element.avg)),
              percent: roundToTwoDecimalPlaces(Number(item.percent) + Number(element.percent)),
