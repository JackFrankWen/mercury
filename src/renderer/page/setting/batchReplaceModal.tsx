...
const BatchReplaceModal: React.FC<BatchReplaceModalProps> = ({ visible, rule, onClose, onSuccess }) => {
  ...
  const steps = [
    {
      title: '设置条件',
      content: (
        <Form form={form} layout="vertical" initialValues={{ dateRange: [dayjs().startOf('year'), dayjs().endOf('year')] }}>
          <Form.Item name="dateRange" label="交易时间范围" rules={[{ required: true, message: '请选择时间范围' }]}>
